...
static toy_status raylib_init_window(toy_state *state) {
    ...
    if (IsWindowReady()) {
        return toy_error(state, "raylib.init-window window is already open");
    }
    ...
    if (!title) {
        ...
        return toy_error(state, "raylib.init-window could not copy the title");
    }
    if (!toy_pop(state, 3)) {
        free(title);
        return toy_error(state, "raylib.init-window failed to pop its inputs");
    }

    InitWindow(width, height, title);
    free(title);
    if (!IsWindowReady()) {
        return toy_error(state,
                         "raylib.init-window failed to initialize the window");
    }
    return TOY_OK;
}
...
static toy_status raylib_set_target_fps(toy_state *state) {
    ...
    if (!toy_pop(state, 1)) {
        return toy_error(state,
                         "raylib.set-target-fps failed to pop its input");
    }
    SetTargetFPS(fps);
    return TOY_OK;
}
...
static toy_status raylib_rgba(toy_state *state) {
    ...
    if (!toy_pop(state, 4)) {
        return toy_error(state, "raylib.rgba failed to pop its inputs");
    }
...
static toy_status raylib_clear_background(toy_state *state) {
    ...
    if (!get_color(state, 0, &color)) {
        return toy_error(state,
                         "raylib.clear-background expected an RGBA color");
    }
    if (!toy_pop(state, 1)) {
        return toy_error(state,
                         "raylib.clear-background failed to pop its input");
    }
...
static toy_status raylib_draw_rectangle(toy_state *state) {
    ...
    if (!toy_pop(state, 5)) {
        return toy_error(state,
                         "raylib.draw-rectangle failed to pop its inputs");
    }
...
static toy_status raylib_draw_text(toy_state *state) {
    ...
    if (!text) {
        ...
        return toy_error(state, "raylib.draw-text could not copy the text");
    }
    if (!toy_pop(state, 5)) {
        free(text);
        return toy_error(state,
...
toy_status toy_raylib_register(toy_state *state) {
    return toy_register_native_module(state, &raylib_module);
}
