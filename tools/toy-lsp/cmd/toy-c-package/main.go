...
func run(args []string, stdout, stderr io.Writer) int {
	...
	root, err := sdkroot.Resolve(parsed.sdkRoot, "include", "toy_package.h")
	if err != nil {
		fmt.Fprintf(stderr, "toy-c-package: %v\n", err)
		return 1
	}
	...
	nativeFile := "toy_" + packageName + sharedLibrarySuffix(runtime.GOOS)
	output := filepath.Join(packageDirectory, nativeFile)
	...
	if err := runCommand(compile, stdout, stderr); err != nil {
		return reportCommandError(stderr, "compile C package", err)
	}
	if err := runCommand(link, stdout, stderr); err != nil {
		return reportCommandError(stderr, "link C package", err)
	}

	manifest := filepath.Join(packageDirectory, "toy.package")
	contents := []byte(fmt.Sprintf("name = %s\nnative = %s\n",
		packageName, nativeFile))
	if err := os.WriteFile(manifest, contents, 0o644); err != nil {
		fmt.Fprintf(stderr, "toy-c-package: write %s: %v\n", manifest, err)
		return 1
	}
	fmt.Fprintf(stdout, "built C package library %s\n", output)
...
func printUsage(output io.Writer) {
	...
	fmt.Fprintln(output)
	fmt.Fprintln(output, "Compile one C adapter into a C-backed Toy package and write toy.package.")
	fmt.Fprintln(output)
