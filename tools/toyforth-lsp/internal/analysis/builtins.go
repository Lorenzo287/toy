...
var builtinDocs = map[string]builtinDoc{
	...
	"%":            {StackEffect: "a b -- remainder", Description: "Compute the truncating remainder of integer division."},
	"mod":          {StackEffect: "a b -- modulo", Description: "Compute Euclidean modulo, returning a non-negative result."},
	...
	"error":        {StackEffect: "string --", Description: "Raise a runtime error using a string message."},
	...
	"split-mid":    {StackEffect: "vector|list|string -- left right", Description: "Split a vector, list, or string into left and right halves."},
	...
	"print":        {StackEffect: "x --", Description: "Print one value literally, followed by a newline."},
	"printf":       {StackEffect: "x -- | args... format --", Description: "Print one value without a newline, interpreting {} placeholders only when the top value is a format string."},
	...
	"read-key":     {StackEffect: "-- char", Description: "Read one byte from input as a one-character string."},
	"read-line":    {StackEffect: "-- string", Description: "Read one line from stdin without its trailing newline."},
	"clear":        {StackEffect: "--", Description: "Clear the terminal screen."},
	"type-of":      {StackEffect: "x -- string", Description: "Push a string naming the input type."},
	...
	"search-words": {StackEffect: "query -- vector", Description: "Push a sorted vector of quoted symbols whose names or docs match query."},
	"repr":         {StackEffect: "x -- string", Description: "Return a source-style representation of a value, with string bytes escaped."},
	"load":         {StackEffect: "path --", Description: "Load and execute a Toy source file in the current context."},
	"read-file":    {StackEffect: "path -- string", Description: "Read the entire content of a file into a string."},
	"read-lines":   {StackEffect: "path -- vector", Description: "Read a file and return a vector of its lines."},
	"write-file":   {StackEffect: "path content --", Description: "Write string content to a file."},
	"delete-file":  {StackEffect: "path --", Description: "Delete a file."},
	"file-exists?": {StackEffect: "path -- bool", Description: "Check if a file exists."},
	...
	">string":      {StackEffect: "vector|list|string -- string", Description: "Build a string from a sequence of one-character strings."},
	...
	"contains?":    {StackEffect: "vector|list item -- bool | string substring -- bool", Description: "Check item membership in a vector/list or substring presence in a string."},
	"index-of":     {StackEffect: "vector|list item -- idx|-1 | string substring -- idx|-1", Description: "Return the first item or substring index, or -1 when absent."},
	...
	"get":          {StackEffect: "map key -- value", Description: "Fetch a map value. Missing keys are runtime errors; use get-or or has? when absence is expected."},
	"get-or":       {StackEffect: "map key default -- value", Description: "Fetch a map value, returning the supplied default when the key is absent."},
	...
	"skip":         {StackEffect: "vector|list|string n -- vector|list|string", Description: "Return a vector, list, or string with the first n items or characters removed."},
	"len":          {StackEffect: "collection -- n", Description: "Get the length of a vector, list, string, map, set, deque, or priority queue."},
	"first":        {StackEffect: "vector|list|string|deque -- x|char", Description: "Consume a non-empty ordered collection and return its first item."},
	"last":         {StackEffect: "vector|list|string|deque -- x|char", Description: "Consume a non-empty ordered collection and return its last item."},
	...
	">char":        {StackEffect: "n -- char", Description: "Construct a one-character string from an integer code between 0 and 255."},
	"char-code":    {StackEffect: "char -- n", Description: "Return the unsigned integer code of a one-character string."},
	...
	"get-env":      {StackEffect: "key -- value", Description: "Read an environment variable. Fails when it is not set; use env? when absence is expected."},
	"set-env":      {StackEffect: "key value --", Description: "Set an environment variable, failing if the operation is rejected."},
	"pwd":          {StackEffect: "-- string", Description: "Push the current working directory."},
	"shell":        {StackEffect: "cmd -- output status", Description: "Run a shell command and push its output followed by its exit status."},
	"unix-time":    {StackEffect: "-- seconds", Description: "Push whole seconds since 1970-01-01 00:00:00 UTC."},
	"local-time":   {StackEffect: "-- map", Description: "Push the current local year, month, day, hour, minute, and second."},
	"utc-time":     {StackEffect: "-- map", Description: "Push the current UTC year, month, day, hour, minute, and second."},
	"cpu-time":     {StackEffect: "-- seconds", Description: "Push process CPU time in seconds as a float."},
	"monotonic-ns": {StackEffect: "-- nanoseconds", Description: "Push a monotonic wall-clock reading in integer nanosecond units; actual resolution is platform-dependent."},
