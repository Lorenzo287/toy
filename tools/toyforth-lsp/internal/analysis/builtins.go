...
var builtinDocs = map[string]builtinDoc{
	...
	"key":          {StackEffect: "-- char", Description: "Read one byte from input as a one-character string."},
	"input":        {StackEffect: "-- string", Description: "Read one line from stdin without its trailing newline."},
	...
	"indexof":      {StackEffect: "vector|list item -- idx|-1 | string substring -- idx|-1", Description: "Return the first item or substring index, or -1 when absent."},
