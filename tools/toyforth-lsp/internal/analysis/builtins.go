...
var builtinDocs = map[string]builtinDoc{
	...
	"filter":    {Signature: "filter ( list pred -- list )", Description: "Keep list items for which a predicate quotation returns true."},
	"some":      {Signature: "some ( list pred -- bool )", Description: "Check whether any list item satisfies a predicate quotation."},
	"all":       {Signature: "all ( list pred -- bool )", Description: "Check whether every list item satisfies a predicate quotation."},
	"split":     {Signature: "split ( list pred -- true false ) | ( str sep -- list )", Description: "Partition a list with a predicate quotation, or split a string using a separator."},
	"splitmid":  {Signature: "splitmid ( list -- left right )", Description: "Split a list into left and right halves."},
	"merge":     {Signature: "merge ( list1 list2 pred -- merged )", Description: "Merge two sorted lists. The predicate must leave exactly one boolean and the surrounding stack is preserved."},
	...
	"typeof":    {Signature: "typeof ( x -- x str )", Description: "Push a string naming the type of the top stack item without consuming it."},
	"bool?":     {Signature: "bool? ( x -- x bool )", Description: "Check whether the top stack item is a boolean without consuming it."},
	"int?":      {Signature: "int? ( x -- x bool )", Description: "Check whether the top stack item is an integer without consuming it."},
	"float?":    {Signature: "float? ( x -- x bool )", Description: "Check whether the top stack item is a float without consuming it."},
	"str?":      {Signature: "str? ( x -- x bool )", Description: "Check whether the top stack item is a string without consuming it."},
	"symbol?":   {Signature: "symbol? ( x -- x bool )", Description: "Check whether the top stack item is a symbol without consuming it."},
	"list?":     {Signature: "list? ( x -- x bool )", Description: "Check whether the top stack item is a list without consuming it."},
	"number?":   {Signature: "number? ( x -- x bool )", Description: "Check whether the top stack item is an integer or float without consuming it."},
	"nan?":      {Signature: "nan? ( x -- x bool )", Description: "Check whether the top stack item is a NaN float without consuming it."},
	"inf?":      {Signature: "inf? ( x -- x bool )", Description: "Check whether the top stack item is an infinite float without consuming it."},
	"word?":     {Signature: "word? ( name -- name bool )", Description: "Check whether a symbol or string names a dictionary word without consuming it."},
	"var?":      {Signature: "var? ( name -- name bool )", Description: "Check whether a symbol or string names a visible dynamic capture without consuming it."},
	...
	"geth":      {Signature: "geth ( list idx -- list value ) | ( str idx -- str char )", Description: "Fetch a list element or one-character string by index without consuming the collection."},
	"seth":      {Signature: "seth ( list idx value -- ) | ( str idx char -- str )", Description: "Store a list element in place, or return a string with one character replaced."},
	...
	"shell":     {Signature: "shell ( cmd -- output|[] )", Description: "Run a shell command and push its output, or an empty list when there is no output."},
